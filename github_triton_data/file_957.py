...
from neuralcompression.meta.pyjuice.nodes import SumNodes
...
def flatten_sum_nodes(ns: SumNodes, *args, use_cuda: bool = False):
    ...
    if not ns.is_tied():
        return (
            ...
            edge_ids,
            ns._param_range,
            [(c.num_nodes, c._output_ind_range) for c in ns.chs],
            *args,
        )
    else:
...
def next_power_of_2(x: torch.Tensor):
    return torch.where(x == 1, 1, 2 ** torch.ceil(torch.log2(x.float())).long())
...
@torch.no_grad()
def sum_layer_forward_compilation_job(
    flat_nodes,
    nids,
    cids,
    pids,
    ...
    for ns_idx, flat_ns in enumerate(flat_nodes):
        ...
        if add_params_flag:
            ns_param_ids = torch.zeros(
                [edge_ids.size(1)], dtype=torch.long, device=edge_ids.device
            )
        ...
        nid_end = (
            ns_num_nodes
            if node_start + ns_num_nodes <= job_end
            else job_end - node_start
        )
        ...
        for nid in range(nid_start, nid_end):
            ...
            for cnode_id, flat_cs in enumerate(flat_ns[3]):
                ...
                if add_params_flag:
                    ...
                    curr_param_ids = torch.arange(
                        curr_ids.size(0), device=edge_ids.device
                    ) + (ns_pid_start + ns_local_pid + ch_start)
            ...
            parids = torch.arange(ch_start, device=edge_ids.device) + (
                ns_pid_start + ns_local_pid
            )
...
@torch.no_grad()
def sum_layer_forward_compilation_legacy(
    nodes,
    fw_group_max_chs,
    n_group_ids,
    n_id_in_group,
    ...
    nids = [
        torch.zeros([group_size], dtype=torch.long) for group_size in num_ns_in_group
    ]  # Node id
    cids = [
        torch.zeros([group_size, max_chs], dtype=torch.long)
        for group_size, max_chs in zip(num_ns_in_group, fw_group_max_chs)
    ]  # Child id
    pids = [
        torch.zeros([group_size, max_chs], dtype=torch.long)
        for group_size, max_chs in zip(num_ns_in_group, fw_group_max_chs)
    ...
    ch_n_pars = torch.zeros(
        [ch_prod_layer_size], dtype=torch.long
    )  # Number of parents for each child node
    for ns in nodes:
        ch_start = 0
        for cs in ns.chs:
            ...
            c_ns_counts = torch.bincount(
                ns.edge_ids[1, criterion] - ch_start, minlength=cs.num_nodes
            )
    ...
    for ns, add_params_flag in zip(nodes, add_ns_params_flag):
        if add_params_flag:
            ns._param_range = (
                ns._param_ids.min().item(),
                ns._param_ids.max().item() + 1,
            )
...
@triton.jit
def _assign_target_ncpids_kernel(
    target_nids_ptr,
    nids_group_start_ptr,
    target_cids_ptr,
    pcids_group_start_ptr,
    target_pids_ptr,
    edge_ids_ptr,
    chs_offsets_ptr,
    n_group_ids_ptr,
    ...
    cs_ele_id_start_ptr,
    cs_node_cum_ids_ptr,
    fw_group_max_chs_ptr,
    cum_n_chs_ptr,
    ns_param_ids_ptr,
    ch_n_pars_ptr,
    constexprs_ptr,
    num_chs: tl.constexpr,
    add_params_flag: tl.constexpr,
    ...
    cid_node_id = (
        tl.sum(
            tl.broadcast_to(cid[:, None], (BLOCK_SIZE, num_chs))
            >= tl.broadcast_to(cs_node_cum_ids[None, :], (BLOCK_SIZE, num_chs)),
            axis=1,
        )
        - 1
    )
...
@torch.no_grad()
def sum_layer_forward_compilation(
    nodes,
    fw_group_max_chs,
    n_group_ids,
    n_id_in_group,
    ...
    pcids_group_start[1:] = torch.cumsum(
        (num_ns_in_group * fw_group_max_chs)[:-1], dim=0
    )
    target_cids = torch.zeros(
        [(num_ns_in_group * fw_group_max_chs).sum()], dtype=torch.long
    ).cuda()

    # ...and `pids`
    target_pids = torch.zeros(
        [(num_ns_in_group * fw_group_max_chs).sum()], dtype=torch.long
    ).cuda()
    ...
    for ns_idx, ns in enumerate(nodes):
        ...
        constexprs = (
            torch.tensor([global_nid_start, ns_pid_start, node_start, ns_num_edges])
            .long()
            .cuda()
        )
...
@torch.no_grad()
def sum_layer_backward_compilation_legacy(
    nodes,
    pids,
    fw_n_group_ids,
    fw_n_id_in_group,
    ...
    ch2flatidx = (
        group2flatidx[bk_n_group_ids]
        + bk_n_id_in_group * bk_group_max_pars[bk_n_group_ids]
    )
    ...
    for ns in nodes:
        ...
        for nid in range(ns.num_nodes):
            ...
            for cnode_id, cs in enumerate(ns.chs):
                ...
                flat_cids = (
                    ch2flatidx[ch_ids] + par_counts[ch_ids]
                )  # start position specified by `ch2flatidx` + offset specified by `par_counts`
    ...
    for group_id in range(num_bk_groups):
        ...
        parids.append(
            flat_parids[flatid_start:flatid_end]
            .reshape(group_size, max_n_par)
            .contiguous()
        )
        parpids.append(
            flat_parpids[flatid_start:flatid_end]
            .reshape(group_size, max_n_par)
            .contiguous()
        )
...
@triton.jit
def _assign_global_eleids_kernel(
    global_ele_ids_ptr,
    cs_ele_id_start_ptr,
    cs_node_cum_ids_ptr,
    edge_ids_ptr,
...
@triton.jit
def _assign_target_parids_kernel(
    target_parids_ptr,
    target_parpids_ptr,
    parids_group_start_ptr,
    flat_pids_ptr,
    pids_group_start_ptr,
    edge_ids_ptr,
    global_ele_ids_ptr,
    chs_offsets_ptr,
    par_offsets_ptr,
    fw_n_group_ids_ptr,
    fw_n_id_in_group_ptr,
    bk_n_group_ids_ptr,
    bk_n_id_in_group_ptr,
    fw_group_max_chs_ptr,
    bk_group_max_pars_ptr,
    constexprs_ptr,
    BLOCK_SIZE: tl.constexpr,
):
    pid = tl.program_id(axis=0)
...
@torch.no_grad()
def sum_layer_backward_compilation(
    nodes,
    pids,
    fw_n_group_ids,
    fw_n_id_in_group,
    ...
    parids_group_start[1:] = torch.cumsum(
        (bk_num_ns_in_group * bk_group_max_pars)[:-1], dim=0
    )
    target_parids = torch.zeros(
        [(bk_num_ns_in_group * bk_group_max_pars).sum()], dtype=torch.long
    ).cuda()

    # Do the same to `parpids`
    target_parpids = torch.zeros(
        [(bk_num_ns_in_group * bk_group_max_pars).sum()], dtype=torch.long
    ).cuda()
    ...
    pids_group_start[1:] = torch.cumsum(
        (fw_num_ns_in_group * fw_group_max_chs)[:-1], dim=0
    )
    flat_pids = torch.zeros(
        [(fw_num_ns_in_group * fw_group_max_chs).sum()], dtype=torch.long
    )
    sid = 0
    for group_id, (gsize, gnchs) in enumerate(
        zip(fw_num_ns_in_group, fw_group_max_chs)
    ):
        eid = sid + (gsize * gnchs)
    ...
    for ns in nodes:
        ...
        # Saves 99.9% compilation time :)
        constexprs = (
            torch.tensor([global_nid_start, node_start, ns_num_edges]).long().cuda()
        )
...
@torch.no_grad()
def prod_layer_forward_compilation(
    nodes,
    fw_group_max_chs,
    n_group_ids,
    n_id_in_group,
    ...
    for ns_id, ns in enumerate(nodes):
        ...
        for cs_id, cs in enumerate(ns.chs):
            cids[group_id][local_sid:local_eid, cs_id] = (
                ns.edge_ids[:, cs_id] + cs._output_ind_range[0]
            )
...
def flatten_c_ids(nids, cids):
    ...
    for curr_nids, curr_cids in zip(nids, cids):
        ...
        flat_cid2nid[c_sid:c_eid] = (
            curr_nids.unsqueeze(1).repeat(1, curr_cids.size(1)).reshape(-1)
        )
...
@njit
def _assign_cid2_group_local_id(
    flat_u_cids, n_group_ids, n_id_in_group, cid2group_id, cid2local_id
):
...
@triton.jit
def _assign_target_ucids_kernel(
    target_u_cids_ptr,
    flat_u_cids_ptr,
    n_group_ids_ptr,
    n_id_in_group_ptr,
    u_cids_group_start_ptr,
    constexprs_ptr,
    BLOCK_SIZE: tl.constexpr,
):
    pid = tl.program_id(axis=0)
...
@triton.jit
def _assign_prod_target_parids_kernel(
    target_parids_ptr,
    flat_cid2nid_ptr,
    flat_cids_ptr,
    cid2group_id_ptr,
...
@torch.no_grad()
def prod_layer_backward_compilation(
    flat_u_cids,
    flat_cids,
    flat_cid2nid,
    bk_group_max_pars,
    ...
    if use_cuda:
        ...
        parids_group_start[1:] = torch.cumsum(
            (num_ns_in_group * bk_group_max_pars)[:-1], dim=0
        )
        target_parids = torch.zeros(
            [(num_ns_in_group * bk_group_max_pars).sum()], dtype=torch.long
        )
        ...
        u_cids = [
            torch.zeros([group_size], dtype=torch.long)
            for group_size in num_ns_in_group
        ]  # Node id
        parids = [
            torch.zeros([group_size, max_n_pars], dtype=torch.long)
            for group_size, max_n_pars in zip(num_ns_in_group, bk_group_max_pars)
        ]  # Parent id
