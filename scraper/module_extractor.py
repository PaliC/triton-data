...
def extract_triton_jit_functions(zip_file_path):
    ...
    runnable_functions = [
        (node.name, generate_runnable_code(node)) for node in triton_function_nodes
    ]
...
if __name__ == "__main__":
    ...
    for func_name, runnable_code in functions:
        # print(f"File: {file_name}, Function: {func_name}")
        try:
            exec(runnable_code, {})
            print(f"Successfully ran the function '{func_name}'")
        except Exception as e:
            print(f"Failed to run the function '{func_name}': {str(e)}")
